				...
				<thead>
				  <tr>
					...
					<c:forEach items="${classList}" var="c">
						<th >
							<a rel="tooltip" title='${c.cname}(0-${c.cmax}人)<c:if test="${not empty c.cremark}">(${c.cremark})</c:if>'>
								${c.cname}(0-${c.cmax}人)
							</a>
						</th>
					</c:forEach>
						...
						<tr>
							...
							<c:forEach items="${t.reservations }" var="r" varStatus="status">
								<td>
									...
									<%-- 可预约 --%>
									<c:if test="${empty r}">
										<a class="forAdd" title="${classList[status.index].cname} <fmt:formatDate value='${t.timewuantum.tbegintime}' pattern='HH:mm'/>-<fmt:formatDate value='${t.timewuantum.tendtime}' pattern='HH:mm'/>" rel="tooltip" href="/reservation/user.do?flag=forAddReservation&cid=${classList[status.index].cid}&date_=<fmt:formatDate value='${rm.date}' pattern='yyyy-MM-dd'/>&tid=${t.timewuantum.tid}">
											<strong class="text-center">预约</strong>
										</a>
									</c:if>
									<%-- 可查询 --%>
									<c:if test="${not empty r}">
										<a class="show" title="${r.cname} <fmt:formatDate value='${t.timewuantum.tbegintime}' pattern='HH:mm'/>-<fmt:formatDate value='${t.timewuantum.tendtime}' pattern='HH:mm'/> ${r.rspecificcontent }，负责人：${r.rchargeperson}，负责人电话：${r.rchargetelephone}" rel="tooltip" href="/reservation/user.do?flag=showReservation&rid=${r.rid}">
